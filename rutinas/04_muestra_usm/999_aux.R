...
base_ocupada <- base_ocupada %>% 
  ...
  arrange(man, n_umce) %>% 
  mutate(id_upm = case_when(id_conglomerado == "1101500063" & 
                              (man == "003" | n_umce <= "114") ~ "110150006301", 
                            id_conglomerado == "1101500063" & n_umce >= "115" ~ "110150006302", 
                            T ~ id_upm), 
         semana_nueva = case_when(id_upm == "110150006301" ~ 28, 
                                  id_upm == "110150006302" ~ 27, 
                                  T ~ semana_nueva)) %>%
...
marco_viv_superman <- marco_viv_superman %>% 
  mutate(grupo = case_when(id_conglomerado == "1101500063" & 
                             (man == "003" | n_umce <= "114") ~ 1, 
                           id_conglomerado == "1101500063" & n_umce >= "115" ~ 2, 
                           T ~ grupo))
