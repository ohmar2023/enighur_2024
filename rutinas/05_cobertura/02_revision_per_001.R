...
#   
# # -----------------------------------------------------------------------------
# # Muestra semana 1 y 2
# # -----------------------------------------------------------------------------
# 
# muestra_sem_1_2 <- read_excel("productos/02_muestra_usm/periodo_01/muestra_myc.xlsx") %>% 
...
# # -----------------------------------------------------------------------------
# 
# muestra_sem_4 <- read_excel("productos/02_muestra_usm/periodo_01/muestra_myc_semana4.xlsx") %>% 
#   mutate(zonal, man_nloc = ifelse(zona == "999", "000", manzana),
#          id_edif = paste0(provin, canton, parroq, zona, sector, man_nloc, num_edif),
#          id_viv = paste0(provin,canton,parroq,zona,sector,man_nloc,num_edif, numviv)) %>% 
#   #filter(semana %in%  c(1,2)) %>% 
#   select(zonal, id_upm, provin, canton, parroq,zona,sector,man_nloc,num_edif, numviv,id_edif,id_viv,
...
muestra_per_1 <- read_excel("productos/02_muestra_usm/periodo_01/muestra_periodo_01_myc.xlsx") %>% 
     mutate(zonal, man_nloc = ifelse(zona == "999", "000", manzana),
            id_edif = paste0(provin, canton, parroq, zona, sector, man_nloc, num_edif),
            id_viv = paste0(provin,canton,parroq,zona,sector,man_nloc,num_edif, numviv)) %>% 
     #filter(semana %in%  c(1,2)) %>% 
...
# muestra_per_1 <- rbind(muestra_sem_1_2,
#                        muestra_sem_3,
#                        muestra_sem_4) 

muestra_per_1 %>% group_by(semana) %>% summarise(n())

# -----------------------------------------------------------------------------
# Validación muestra periodo 1 con cobertura periodo 1
# -----------------------------------------------------------------------------

revision <- muestra_per_1 %>% 
  full_join(cobertura_base , by = "id_viv") %>% 
  mutate(control = ifelse(is.na(pro.x) | is.na(pro.y),1,0)) %>% 
  filter(control == 1) 

viv_muestra_no_cober <- muestra_per_1 %>% filter(!id_viv %in% cobertura_base$id_viv)

viv_cober_no_muestra <- cobertura_base %>% filter(!id_viv %in% muestra_per_1$id_viv)
