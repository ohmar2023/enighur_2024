...
Se presentan los resultados acorde a la información recolectada desde el periodo 01 hasta el periodo `r periodo`. A lo largo del documento se hace referencia y se presentan los resultados desagregados por provincia y por Coordinación Zonal: Administración Central Campo (Adm.C.Campo), Centro, Litoral y Sur. 
...
Desagregando los resultados para cada una de las coordinaciones zonales podemos evidenciar que las que presentan más inconvenientes con respecto a "rechazos" son la coordinación Litoral y Adm.C.Campo.
...
Se presenta la información de las tasas de conformidad en cada periodo (periodo 01 - periodo `r periodo`). Podemos evidenciar que los periodos más deficientes a nivel de cobertura son el 01 y el 02. El periodo 07 presenta un retroceso con respecto a la calidad de cobertura de los dos periodos que lo preceden.
...
Con el objetivo de evaluar el comportamiento de las tasas de conformidad por provincia, se presenta el gráfico 4 en el que podemos apreciar que la provincia de Pichincha, Galápagos y Santo Domingo se mantienen por debajo del umbral establecido. La provincia de Guayas, a pesar de estar sobre el umbral, requiere la misma atención que las otras provincias mencionadas. 
...
El presente apartado reúne la información de las tasas de conformidad a nivel de UPM, es decir, se evalúa los umbrales en los que se encuentran y se identifica la manera en la que están conformadas las UPM de acuerdo al resultado de la encuesta de cada una de las viviendas que la conforman.

En la siguiente tabla se presentan los resultados desagregados por coordinación zonal.
...
tabla_tasas_conf_upm_zonal_aux <- tabla_tasas_conf_upm_zonal %>% 
  ...
  pivot_wider(id_cols = c("zonal"), names_from = TRE, values_from = porcentaje) 

tabla_tasas_conf_upm_zonal_aux[is.na(tabla_tasas_conf_upm_zonal_aux)] <- "0.00%"

tabla_tasas_conf_upm_zonal_aux
